<h1> hello </h1>
<?php
session_start();
require 'database.php';
date_default_timezone_set('America/Chicago');
$token = $_POST['token'];
$story_id = $_POST['story_id'];
$date = date("Y-m-d g:i:s");
// we will first check if the user can even access this page. if the user is not logged in, they will
// immediately be redirected to homepage and they cant see the contents of this page at all
// an extra security check is also done by verifying the session token and the token posted
// from the previous page the user was on 
if(!isset($_SESSION['username'])){ 
    header("Location: homepage2.php");
    exit;
} 
else if(!hash_equals($_SESSION['token'], $token)){
    die("You are accessing a page you do not have acccess to");
}
// editted comment is added to database
$stmt = $mysqli->prepare("update comments set comment = ?, commenttime = ? where comment_id = ?");
if(!$stmt){
    printf("Query Prep Failed: %s\n", $mysqli->error);
    exit;
}
$stmt->bind_param('sss', $_POST['newcomment'], $date, $_POST['comment_id']);
$stmt->execute();
$stmt->close();
echo "Your comment has been successfully edited. Please click back and refresh the page to see the changes. <br>";
echo "<button onclick='window.history.go(-2)'>Back</button>";
?>